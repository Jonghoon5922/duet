# Duet

나와 Claude가 함께 하는 일을 타스크로 묶고, 세션이 다 끝나면 타스크가 닫히는 **개인용 로컬 PMS**. MCP로 세션이 상태를 보고하고, 로컬 대시보드에서 사람이 보고 고친다. 패키지 `duet-mcp`, CLI `duet`.

**작업 시작 전 `SPEC.md`를 먼저 읽는다.** 서재(`C:\project\bookshelf`)와 같은 스택·같은 배포 방식이므로 그쪽 코드(FastMCP 서버, 웹 UI, `app` 창, 인스톨러, watchdog)를 재사용한다.

## 만드는 방식

**프로토타입 먼저, 뼈대를 붙여 나간다.** 단계마다 되는 것을 먼저 만들고, 필요해질 때 살을 붙인다.
처음부터 계층을 나누지 않는다 — 지금은 `core.py` 하나가 파일과 규칙 전부다.

## 핵심 원칙

- 이 프로젝트 코드는 **LLM을 호출하지 않는다.** 진행 로그도 Claude가 `report_progress`로 직접 쓴다.
- **MCP 서버 프로세스 = 세션.** 서버가 뜨면 세션 파일 생성, 하트비트, 종료 시 `중지` 기록.
- **DB 없음. 로컬 파일만.** `~/.duet/<프로젝트>/T001-…/`. 세션 파일은 자기 프로세스만 쓴다. 타스크 상태는 저장하지 않고 세션에서 계산한다. 프로젝트는 폴더, 번호는 프로젝트 안에서 1부터.
- **`task.md`는 편집만 쓴다** — 사람도, Claude의 `update_task`도. 자동으로 도는 것은 절대 안 쓴다.
- **살아 있는 세션은 아무도 못 건드린다.** 끝난 세션만 사람이 완료/중지로 바꾼다.
- **모든 일은 타스크를 타고 간다.** 사용자가 자연어로 시키면 Claude가 스스로 join/create → report → complete. 계획을 보여주는 순간 타스크를 만든다. 제목·설명·로그는 사용자가 쓰는 말로, 사람 메모처럼.
- **타스크 하나에 살아 있는 세션 하나, 세션 하나에 타스크 하나.** 이어받기는 앞 세션이 끝난 뒤. 한 창이 타스크를 순서대로 여러 개 하면 그때마다 새 세션이다. 프로젝트 안에서는 창 여럿이 각자 다른 타스크를 한다.
- **자동 완료는 세션이 전부 `완료`일 때만.** 끊긴 세션이 남으면 `확인 필요` — 사람이 판단한다. 사람이 고르는 것은 `완료`·`보류`·`취소` 셋뿐이다.
- **사람이 정한 것이 자동 규칙보다 우선한다.** `task.md`의 `상태:` 줄 한 줄이 그것이고, 지우면 다시 센다.
- **개인 전용.** 팀 동기화·공유 서버는 만들지 않는다.
- 로컬 우선: `~/.duet/` 폴더 하나, 네트워크 없음, UI는 127.0.0.1만.

## 스택

Python 3.11+, uv, FastMCP, watchdog, FastAPI + SSE, 정적 HTML 한 장

## 진행 상태

- [x] 스펙 초안 v2 (SPEC.md — 파일 기반 저장, Sprintra 확인 절 포함)
- [x] 0단계: Sprintra 확인은 건너뛰기로 결정 (SPEC.md §0에 기록). 내 기준으로 만든다
- [x] 1단계 프로토타입: core.py 하나에 파일 구조와 규칙, MCP 도구 5개 (stdio 실측 13건 통과)
- [x] 2단계: `get_task`·`pause_session`·`update_task` (인수인계 실측). instructions 주입·중지 감지는 1단계에서 끝냄
- [x] 3단계: 대시보드 — 집계 타일·필터·검색·카드 그리드·세션 로그 펼치기·인라인 편집·새 타스크 (폴링 2초, SSE는 아직)
- [x] 4단계 기본: 세션 상태 수동 변경·타스크 닫기·보관/되돌리기 (브라우저 실동작 확인)
- [x] 프로젝트 단위: `~/.duet/<프로젝트>/T001-…` — 창이 뜬 폴더가 프로젝트, 번호는 프로젝트 안에서 1부터, 보관은 폴더째, 옛 배치 자동 이관
- [x] 보드: 프로젝트 상자(+ Task / Archive / ×, 접기), + New Project(이름 + 첫 타스크), 타스크 ×, 글은 영문(상태는 화면에서만)
- [x] 보드: 세션이 일하는 중인 타스크는 잠김, Feedback(GitHub 이슈 화면 채워 열기), 화면 파일이 바뀌면 탭이 스스로 새로고침
- [x] MCP 안내문: 모든 일은 타스크를 타고, 계획을 보여주는 순간 타스크, 글은 사용자 말로·메모처럼
- [x] 빌드는 `scripts/build.py` 한 번 — 테스트·스모크·import·exe 기동을 통과해야 인스톨러가 나온다
- [x] 5단계 인스톨러: PyInstaller 실행 파일 둘 + Inno Setup. 조용히 설치해 등록·MCP·보드 실측
- [x] 보드는 MCP 서버가 겸한다 (창이 열려 있으면 늘 켜짐) + 창 없는 duet-board.exe
- [ ] 4단계 나머지: watchdog + SSE로 폴링 걷어내기. `app` 창은 보드 겸하기로 필요성이 줄었다
- [x] GitHub Release에 인스톨러 (v0.2.0부터)
- [ ] 5단계 나머지: PyPI `duet-mcp`, `.mcpb`, 코드 서명

단계를 끝내면 위 체크박스를 갱신한다.

## 새 세션 시작 문구

```
SPEC.md 읽고 이어서 진행해줘
```
