...
def build_instructions() -> str:
    ...
    lines = [
        ...
        f"이 창의 프로젝트: {here}" if here else (
            ...
            + (", ".join(p for p in core.projects() if p) or "(없음)")
            + ". 비워 두면 만들어지지 않는다."
        ),
        "",
    ]
    ...
    lines += [
        ...
        "제목·설명·진행 로그·마무리는 사용자가 쓰는 말로, 포스트잇에 적듯 짧고 평범하게. 제목은 명사구(X '…을 …한다', X '— 로 나열'), "
        "설명은 한 문장. 사용자가 한국어면 한국어다.",
...
def create_server(cur: Current, lock: threading.Lock) -> MCPServer:
    ...
    @server.tool(
        ...
        description=(
            ...
            "**말**: 사용자가 쓰는 말로. 한국어로 시키면 한국어. 사람이 포스트잇에 적듯 짧고 평범하게 — "
            "기계가 쓴 티(줄표 나열, 괄호 설명, '…한다' 문장체, 용어 나열)가 나면 안 된다.\n"
            "**title**: 명사구 2~6단어. 결과물 이름이지 문장이 아니다. "
            "O: 'README 다시 쓰기', '보드 영문화', '로그인 화면 DBIO 전환', '인스톨러' / "
            "X: '보드 다듬기 — 배지·검색 문구·번호순·버튼 말', '폴더가 없는 창은 프로젝트 이름을 Claude가 정한다', '리팩터링 논의'. "
            "프로젝트 이름은 넣지 마라 (X: 'Duet 대시보드 만들기' / O: '대시보드 만들기'). "
            "세션 하나로 끝날 잔일이면 타스크를 만들지 말고 그냥 해라.\n"
            "**description**: 동료에게 메신저로 한 줄 남기듯. 한 문장, 길어야 두 문장. 왜 하는지·뭐가 되면 끝인지만. "
            "O: 'scratch 폴더에서 연 창은 폴더 이름이 쓸모없으니 Claude가 주제로 짓게.' / "
            "X: '임시 폴더는 프로젝트가 아니다. 그런 창에서 타스크를 만들 때는 … 이름 없이 만들면 거절하고 후보를 보여준다.' "
            "계획을 세웠으면 그 요지가 여기 들어간다 — 요지지 목록이 아니다.\n"
            "**project**: 보통 비워 둔다 — 이 창이 뜬 폴더의 프로젝트에 만들어진다. "
            "다른 프로젝트 일을 대신 만들 때만 적는다. 번호는 그 프로젝트 안에서 1부터 난다. "
            "이 창이 프로젝트 폴더가 아닌 곳에서 떴으면(안내문에 그렇게 적혀 있다) 네가 일의 주제로 짧게 짓는다 — "
            "이미 있는 프로젝트면 그 이름 그대로.\n"
    ...
    def create_task_tool(
        ...
        target = project.strip() or cur.project
        if not target:
            # 임시 폴더에서 뜬 창. 폴더 이름을 프로젝트로 쓰면 'scratch-2026-…' 같은 상자가 생긴다.
            # 그럴 바엔 Claude가 주제로 이름을 짓는 편이 낫다 — 있는 것부터 보여준다.
            return {
                "만들지_않았다": "이 창은 프로젝트 폴더가 아닌 곳에서 떠서 프로젝트를 모른다.",
                "있는_프로젝트": [p for p in core.projects() if p],
                "next": "project에 이름을 넣어 다시 불러라. 있는 프로젝트면 그 이름 그대로, 새 일이면 주제로 짧게 "
                        "(사용자가 부르는 말로). 사용자에게 묻지 않아도 된다 — 네가 정하고 답에 한 줄 밝혀라.",
            }
        try:
            task = core.create_task(title, description, target)
    ...
    @server.tool(
        ...
        description=(
            ...
            "X: 'core.fingerprint(파일 수정 시각 지문) + /api/events SSE + 보드 EventSource로 …' 같은 용어 나열). "
